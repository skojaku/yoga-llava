...
We instruct the extractor to restrict its output to statements supported by the current source chunk and to drop claims that a pose cures or treats a medical condition.
Both restrictions are carried by the extraction prompt alone.
...
Every answer remains traceable to its source evidence.
Table~\ref{tab:domain_qa_config} in Appendix~\ref{app:generation_settings} lists the chunking, extraction, and decoding values used here.
...
The reported $R^2$ and RMSE imply a target standard deviation of $53.24^\circ$, the RMSE of that predictor, against which YogaLLaVA V3 reduces RMSE by 59\%.
